myApp.controller('ProductSearchCtrl', ['$scope', '$rootScope',
    ...
    function($scope, $rootScope, $http, $location,
        ...
        $scope.searchProduct = function(searchData) {
            getProductData.searchProduct(searchData)
                .then(function(data) {
                    ...
                    $scope.currentPage = 1;
                    getProductData.setSearchedData($scope.tabledatas);
                })
                .catch(function(error) {
                    console.log('error');
                })

            $scope.$watch('tabledatas', function() {
                if ($scope.tabledatas) {
        ...
        $scope.exportProduct = function(searchdata) {
            var filteredData = $filter('filter')(searchdata, function(value) {
            ...
            $http.post('/exportProduct', filteredData).
        ...
        $scope.$on('productList', function(event, data) {
            $scope.tabledatas = getProductData.getSearchedData();
            $scope.totalItems = $scope.tabledatas.length;
            $scope.itemsPerPage = 10;
            $scope.currentPage = 1;
            $scope.$watch('tabledatas', function() {
